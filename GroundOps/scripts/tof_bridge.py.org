...
def handle_log_stream(payload):
    # Parse argv (uint32 array) starting after command packet header (8 bytes offset)
    argv_data = payload[8:-6] # Exclude 8-byte packet header & 6-byte tail (CRC + Footers)
    ...
    # Component tag (words 5..8)
    comp_bytes = struct.pack('>4I', argv[5], argv[6], argv[7], argv[8])
    component = comp_bytes.decode('utf-8', errors='ignore').rstrip('\x00')

    # Message string (words 9..)
    if msg_len > 0 and len(argv) >= 9 + ((msg_len + 3) // 4):
        msg_words = argv[9 : 9 + ((msg_len + 3) // 4)]
        msg_bytes = struct.pack(f'>{len(msg_words)}I', *msg_words)[:msg_len]
